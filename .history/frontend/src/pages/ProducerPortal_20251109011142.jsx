...
export function ProducerPortal({ setPage }) {
  ...
  const handleSubmit = async () => {
    setMessage(null);
    setPdfUrl(null); // <-- Make sure you're resetting the pdfUrl state
    ...
    // --- 1. DECLARE 'result' HERE (outside the try block) ---
    let result = null; 
    ...
    try {
      if (mode === "create") {
        const data = {
          ...
          crate_count: parseInt(formData.crate_count),
          grade: formData.grade,
          produce: formData.produce,
        };
        // --- 2. ASSIGN to 'result' (don't declare with 'const') ---
        ...
        setPdfUrl(result.pdf_url); // <-- Set the PDF URL state
      ...
      // --- 3. This logic now works because 'result' is in scope ---
      if (mode === "create") {
